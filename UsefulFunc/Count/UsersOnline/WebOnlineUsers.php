...
namespace UsefulFunc\Count\UsersOnline;

// 统计网站当前在线人数
class WebOnlineUsers
{
    /**
     ...
     */
    public function statisticByIpAndLog($timeout = '1800')
    {
        $filename = $this->createLogFile();
        $temp = [];
    ...
    */
    public function statisticByCookieAndLog($timeout = '1800')
    {
     ...
     */
    public function statisticByRedisBitMap()
    {
        $redis = new Redis();
        // 用户ID
        $id = 1;
     ...
     */
    public function statisticByRedisSet()
    {
        define('ONLINE_MINUTES', 3);
        $redis = new Redis();
        $redis->connect('localhost', 6379) or exit('Unable to connect to Redis');
