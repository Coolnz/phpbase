...
class Read
{
     ...
     * 使用fgetc逐字读取；来降低内存使用；.
     */
    public function readByBite($file)
    {
        $fp = fopen($file, 'r');
...
//	echo $char.PHP_EOL;
     ...
     */
    public function readBySize($file)
    {
        $fp = fopen($file, 'r');
        while (!feof($fp)) {
            // 如果你要使用echo，那么，你会很惨烈...
     ...
     */
    public function readByStream($file)
    {
        $fp = fopen($file, 'r');
        while (!feof($fp)) {
     ...
     */
    public function readByYield($file)
    {
        $fp = fopen($file, 'r');
        try {
            while ($line = fgets($fp)) {
     ...
     * @return Generator
     */
    public function readByYield2($file)
    {
        $fp = fopen($file, 'rb');
        while (false === feof($fp)) {
            //每次只读取1024个字节；
     ...
     * @param $file
     */
    public function writeBySplFileObj($file)
    {
        $str = '';
        for ($a = 0; $a < 50; ++$a) {
            $str .= 'abc, ccc, ddd,';
    ...
    public function getTime()
    {
        ...
        $this->writeBySplFileObj($newFile);
        $end = microtime(true);
     ...
     * @param $newFile
     */
    public function writeToLog($file, $newFile)
    {
