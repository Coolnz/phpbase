...
class Geohash
{
    ...
    public function __construct()
    {
        ...
        for ($i = 0; $i < 32; ++$i) {
            $this->codingMap[mb_substr($this->coding, $i, 1)] = str_pad(decbin($i), 5, '0', STR_PAD_LEFT);
        }
    }
    ...
    public function decode($hash)
    {
        ...
        $hl = mb_strlen($hash);
        for ($i = 0; $i < $hl; ++$i) {
            $binary .= $this->codingMap[mb_substr($hash, $i, 1)];
        }
        ...
        for ($i = 0; $i < $bl; ++$i) {
            if ($i % 2) {
                $blat = $blat . mb_substr($binary, $i, 1);
            } else {
                $blong = $blong . mb_substr($binary, $i, 1);
            }
        }
    ...
    {
        $geohashPrefix = mb_substr($srcHash, 0, mb_strlen($srcHash) - 1);
    ...
    public function encode($lat, $long)
    {
        ...
        while (mb_strlen($blat) + mb_strlen($blong)) {
            if ($uselong) {
                $binary = $binary . mb_substr($blong, 0, 1);
                $blong = mb_substr($blong, 1);
            } else {
                $binary = $binary . mb_substr($blat, 0, 1);
                $blat = mb_substr($blat, 1);
            }
    ...
    {
        $srcHash = mb_strtolower($srcHash);
        $lastChr = $srcHash[mb_strlen($srcHash) - 1];
        $type = (mb_strlen($srcHash) % 2) ? 'odd' : 'even';
        $base = mb_substr($srcHash, 0, mb_strlen($srcHash) - 1);

        if (false !== mb_strpos($this->borders[$dir][$type], $lastChr)) {
            $base = $this->calculateAdjacent($base, $dir);
        }

        return $base . $this->coding[mb_strpos($this->neighbors[$dir][$type], $lastChr)];
    }
    ...
    */
    ...
    private function binDecode($binary, $min, $max)
    {
        ...
        if (0 == mb_strlen($binary)) {
            return $mid;
        }
        $bit = mb_substr($binary, 0, 1);
        $binary = mb_substr($binary, 1);
