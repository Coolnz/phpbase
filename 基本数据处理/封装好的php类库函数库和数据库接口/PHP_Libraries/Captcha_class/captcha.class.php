...
class Captcha
{
    ...
    public function show_captcha()
    {
        ...
        imagefill($im, 0, 0, $bg);
        //绘制边框
        imagerectangle($im, 0, 0, $this->width - 1, $this->height - 1, $color[mt_rand(0, 4)]);

        //随机添加干扰点
        for ($i = 0; $i < 200; ++$i) {
            $radcolor = imagecolorallocate($im, mt_rand(0, 255), mt_rand(0, 255), mt_rand(0, 255));
            imagesetpixel($im, mt_rand(0, $this->width), mt_rand(0, $this->height), $radcolor);
        }

        //随机添加干扰线
        for ($i = 0; $i < 5; ++$i) {
            $radcolor = imagecolorallocate($im, mt_rand(0, 255), mt_rand(0, 255), mt_rand(0, 255));
            imageline($im, mt_rand(0, $this->width), mt_rand(0, $this->height), mt_rand(0, $this->width), mt_rand(0, $this->height), $radcolor);
        }

        //将生成的字符写入图像
        for ($i = 0; $i < $this->number; ++$i) {
            imagettftext($im, 18, mt_rand(-40, 40), 8 + (18 * $i), 24, $color[mt_rand(0, 4)], 'cambriaz.ttf', $this->code[$i]);
        }
    ...
    public function create_string($num, $type)
    {
        ...
        //定制三种类型字符串
        $tmp = [9, 35, mb_strlen($str) - 1];
        //获取字符串
        $char = '';
        for ($i = 0; $i < $num; ++$i) {
            $char .= $str[mt_rand(0, $tmp[$type])];
        }
