...
function get_fileext($path)
{
    ...
    return isset($path[1]) ? mb_strtolower(end($path)) : '';
}
/*
* @todo   获取路径信息
* @param  路径
...
function get_deal_chmod($path, $format = false)
{
    ...
    if (!$format) {
        $mode = mb_substr(sprintf('%o', $perms), -4);
    } else {
        //所有者
        $mode = '';
        $mode .= (($perms & 0x0100) ? 'r' : '-');
...
function uploadFile($filename, $path, $typelist = null)
{
    ...
    if (empty($typelist)) {
        $typelist = ['image/gif', 'image/jpg', 'image/jpeg', 'image/png'];
    }

    $upinfo = ['error' => true, 'info' => ''];
    ...
    do {
        $newfile = date('YmdHis') . mt_rand(1000, 9999) . '.' . $fileinfo['extension'];
